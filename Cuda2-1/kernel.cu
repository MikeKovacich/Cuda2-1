...
int main(int argc, char **argv)
{
	...
	// Graph Parameters
	const unsigned NumNode = 144;		// Number of Nodes
	value_t ProbArc = 0.1;				// Probability that two particles interact
	value_t ProbAttractive = 0.0;		// Probability that the interaction is attractive
	...
	// Arc Parameters
	value_t ForceAlpha = 0.001;			// Coefficient in force model
	...
	if (CPU){
		...
		for (unsigned t = 0; t < NumSteps; t++) {
			...
			arc.Step_CPU(arcData_Host, nodeData_Host);
			node.Step_CPU(nodeData_Host);
